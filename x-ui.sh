...
if [[ x"${release}" == x"centos" ]]; then
    if [[ ${os_version} -le 6 ]]; then
        LOGE "Please use the Centos 7 or higher version of the system！\n" && exit 1
    fi
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/dilathedevilz/ngeteh/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
reset_config() {
    ...
    echo -e "All panel settings have been reset to the default value. Now please restart the panel and use the default ${green}54321${plain} 端口访问面板"
    confirm_restart
}
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "After the setting port is complete, please restart the panel and use the newly set port ${green}${port}${plain} Access panel"
        confirm_restart
    fi
}
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/dilathedevilz/ngeteh/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    ...
    LOGI "This script will use an ACME script application certificate. It needs to be guaranteed when using:"
    LOGI "1.KnowCloudflare register e-mail"
    LOGI "2.Knowing Cloudflare Global API Key"
    LOGI "3.The domain name has passedCloudflare Perform to the current server"
    LOGI "4.The default installation path of the script application certificate is/root/cert directory"
...
show_usage() {
    ...
    echo "x-ui enable       - Set the X-UI boot self-starting"
